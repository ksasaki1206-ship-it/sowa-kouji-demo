import { USERS } from './auth.js?v=20260901-21';
import { generateResidentAccessToken } from './resident-access.js?v=20260901-21';
...
export const DEMO_RESIDENT_TOKENS = Object.freeze({
  c1:'nNAU_zVuA5LTQtEX8vMOm-QaI2nNKBWX', c2:'sBeVRPGqouDZk5FFCAdzUs3iBppeamXe',
  c3:'6MXDgLwG1V0dJH5Tc3E1gt04DU9_ZaXb', c4:'Koh9EW8uTkjzyStcG9_Hm7XQqT-85Dc0',
  c5:'31xxyOrgJF7gP3mXoAKmrBN2t07UD2Bf'
});
...
export function migrateState(raw) {
  ...
  state.responses = Array.isArray(state.responses) ? state.responses.map((r, index) => ({
    ...
    receivedAt: r.receivedAt || new Date().toISOString(), applied: Boolean(r.applied), caseId: r.caseId || '', propertyId:r.propertyId || '', roomId:r.roomId || ''
  })) : [];
  state.cases = Array.isArray(state.cases) ? state.cases.map((c, index) => {
    ...
    residentAccessCreatedAt:c.residentAccessCreatedAt || demo?.residentAccessCreatedAt || '', residentAccessUpdatedAt:c.residentAccessUpdatedAt || '',
  ...
  const usedResidentTokens = new Set();
  state.cases.forEach(item => {
    while (!item.residentAccessToken || usedResidentTokens.has(item.residentAccessToken)) item.residentAccessToken = generateResidentAccessToken();
    usedResidentTokens.add(item.residentAccessToken);
    if (!item.residentAccessCreatedAt) item.residentAccessCreatedAt = new Date().toISOString();
  });
  ...
  state.responses.forEach(response => {
    const linkedCase = state.cases.find(item => item.id === response.caseId)
      || state.cases.find(item => item.property === response.property && normalizeRoomNumber(item.room) === normalizeRoomNumber(response.room));
    if (!linkedCase) return;
    response.caseId = response.caseId || linkedCase.id;
    response.propertyId = response.propertyId || linkedCase.propertyId || '';
    response.roomId = response.roomId || linkedCase.roomId || '';
  });
