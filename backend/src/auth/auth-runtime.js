...
import { createFirebaseIdentityProvider } from './firebase-identity-provider.js';
...
export async function createAuthRuntime(config, businessProvider) {
  ...
  const userStore = config.dataProvider === 'postgres'
    ? createPostgresUserStore({ poolConfig:config.databasePoolConfig, ssl:config.databaseSsl, max:Math.min(config.databasePoolMax, 5) })
    : createMemoryUserStore();
  const identityProvider = config.identityProvider === 'fake'
    ? createFakeIdentityProvider()
    : config.identityProvider === 'firebase'
      ? createFirebaseIdentityProvider({ projectId:config.identityProjectId })
      : createUnconfiguredIdentityProvider();
  const authService = createAuthService({
    userStore, identityProvider, staffStore:businessProvider.staff, auditStore:businessProvider.audit,
    loginProtection:config.loginProtection, publicIdentityConfig:config.identityWebConfig
  });
