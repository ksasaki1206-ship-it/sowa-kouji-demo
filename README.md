...
- `assets/js/repositories.js`: 案件・物件・回答・履歴・工程・ユーザー・担当者・写真のデータ操作
...
住所・管理会社等が旧案件間で異なる場合、物件マスタには最初の非空値だけを仮採用し、案件側の文字列は上書き・削除しません。adminは物件の追加・編集・有効/無効切替、officeは閲覧、workerは管理画面非表示です。本格的な部屋マスタは次工程の対象です。
